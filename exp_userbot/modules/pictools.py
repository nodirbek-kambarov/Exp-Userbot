...
from exp_userbot.helpers.pictool_help import gib_carbon_sar
from exp_userbot.helpers.pyrogram_help import get_arg
from exp_userbot.core.main_cmd import expub_on_cmd, e_or_r
from exp_userbot.core.expub_database.expub_db_conf import get_custom_var
...
@expub_on_cmd(command="carbon", modlue=mod_file)
async def gibcarbon(_, message: Message):
    r_msg = message.reply_to_message
    carbon_msg = await e_or_r(expub_message=message, msg_text="`Processing...`")
    ...
    await EXPUB.send_photo(message.chat.id, carboned_pic)
...
# Background Remover
@expub_on_cmd(command="rmbg", modlue=mod_file)
async def removebg(_, message: Message):
    rmbg_api = await get_rmbg_api()
    rmbg_r_msg = message.reply_to_message
    rmbg_msg = await e_or_r(expub_message=message, msg_text="`Processing...`")
    if rmbg_api is None:
    ...
    if rmbg_header.status_code == requests.codes.ok:
        with open("EXPUB-rmbg.png", "wb") as rmbg_out_image:
            rmbg_out_image.write(rmbg_header.content)
            await EXPUB.send_photo(chat_id=message.chat.id, photo="EXPUB-rmbg.png")
            os.remove("EXPUB-rmbg.png")
    else:
