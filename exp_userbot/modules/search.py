...
**Example:**
  ✘ `duck_s`,
   ⤷ Send command with query = `{Config.CMD_PREFIX}duck_s Exp Userbot`
  ✘ `google`,
   ⤷ Send command with query = `{Config.CMD_PREFIX}google Exp Userbot`
...
@expub_on_cmd(command="duck_s", modlue=mod_file)
async def duckduckg_s(client, message):
    pablo = await e_or_r(expub_message=message, msg_text="`Searcing in DuckDuckGo...`")
...
@expub_on_cmd(command="google", modlue=mod_file)
async def google_s(client, message):
    pablo = await e_or_r(expub_message=message, msg_text="`Searching in Google...`")
    query = get_arg(message)
