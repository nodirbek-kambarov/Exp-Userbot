...
async def ARQ_EXPUB(
    ...
    try:
        ...
        arq_expub = ARQ(arq_url, arq_api, arq_aiohttp)
        # === Now do stuff with arq === #
        # lyrics module
        if is_lyrics:
            lyric = await arq_expub.lyrics(query=keyword)
            await close_session(arq_aiohttp)
            return lyric
        # Translator module
        if is_tr:
            transed = await arq_expub.translate(text=keyword, destLangCode=dest_lang)
            await close_session(arq_aiohttp)
            return transed.result
        # Wiki module
        if is_wiki:
            wiki_s = await arq_expub.wiki(query=keyword)
            await close_session(arq_aiohttp)
            return wiki_s.result
        # Reddit Module
        if is_reddit:
            reddit_s = await arq_expub.reddit(query=keyword)
            await close_session(arq_aiohttp)
            return reddit_s
...
# Lyrics
@expub_on_cmd(command="lyrics", modlue=mod_file)
async def arq_lyrics(_, message: Message):
    lyrics_msg = await e_or_r(expub_message=message, msg_text="`Processing...`")
    ...
    if not keyword:
        ...
        keyword = keyword
    f_lyrics = await ARQ_EXPUB(keyword=keyword, is_lyrics=True)
    nyc_lyrics = f_lyrics.result
    if len(nyc_lyrics) > 4096:
        await lyrics_msg.edit("`Wah!! Long Lyrics tho!, Wait I'm sending it as a file!`")
        lyric_file = open("lyrics_EXPUB.txt", "w+")
        lyric_file.write(nyc_lyrics)
        lyric_file.close()
        await lyrics_msg.reply_document("lyrics_EXPUB.txt")
        os.remove("lyrics_EXPUB.txt")
        await lyrics_msg.delete()
    else:
        await lyrics_msg.edit(nyc_lyrics)

# Translator
@expub_on_cmd(command="tr", modlue=mod_file)
async def arq_trans(_, message: Message):
    trans_msg = await e_or_r(expub_message=message, msg_text="`Processing...`")
    ...
    translate_txt = await ARQ_EXPUB(is_tr=True, keyword=trans_this, dest_lang=dest_l)
    translated_str = f"""
**Translated,**
...
"""
    if len(translated_str) > 4096:
        ...
        tr_txt_file = open("translated_EXPUB.txt", "w+")
        tr_txt_file.write(translated_str)
        tr_txt_file.close()
        await trans_msg.reply_document("translated_EXPUB.txt")
        os.remove("translated_EXPUB.txt")
        await trans_msg.delete()
    else:
        await trans_msg.edit(translated_str)

# Wiki search
@expub_on_cmd(command="wiki", modlue=mod_file)
async def arq_wiki(_, message: Message):
    wiki_msg = await e_or_r(expub_message=message, msg_text="`Processing...`")
    ...
    if not wiki_key:
        ...
        wiki_this = wiki_key
    s_wiki = await ARQ_EXPUB(is_wiki=True, keyword=wiki_this)
    wiki_txt = f"""
**Title:** `{s_wiki.title}`
**Answer:**
`{s_wiki.answer}`
"""
    if len(wiki_txt) > 4096:
        ...
        wiki_txt_file = open("wiki_EXPUB.txt", "w+")
        wiki_txt_file.write(wiki_txt)
        wiki_txt_file.close()
        await wiki_msg.reply_document("wiki_EXPUB.txt")
        os.remove("wiki_EXPUB.txt")
        await wiki_msg.delete()
    else:
        await wiki_msg.edit(wiki_txt)

# Reddit
@expub_on_cmd(command="reddit", modlue=mod_file)
async def arq_reddit(_, message: Message):
    red_msg = await e_or_r(expub_message=message, msg_text="`Processing...`")
    ...
    reddit_now = await ARQ_EXPUB(is_reddit=True, keyword=reddit_this)
    try:
